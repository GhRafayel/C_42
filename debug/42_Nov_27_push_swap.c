...
void to_stack_b(t_stack **stack_a, t_stack **stack_b, int center, int n)
{
	...
	while (n)
	{
		...
		if ((*stack_a) -> val <= center && (*stack_a)-> val != biggist)
		{
	...
	while (n > 2)
	{
		...
		found_doub_op_1(stack_a, stack_b);
		int f = rang((*stack_a)->val, (*stack_b) -> val);
		int l = rang(last_node(*stack_a), (*stack_b) -> val);

		if ( (*stack_a)-> val == biggist)
		{
			ra_rb(stack_a, 'a');
		}
		else if ( l < f)
...
void	to_stack_a(t_stack **stack_a, t_stack **stack_b)
{
	...
	while (stack_size(*stack_b) > 0)
	{
		...
		else
		{
			...
			if ((num != (*stack_a) -> val && (*stack_b) -> val != num ) || (*stack_b) -> val > num)
			{
				if (find_pl(stack_a, num) <= stack_size(*stack_a) / 2)
				{
					if ((*stack_b) -> val < num)
...
int	main(int nn, char **ss)
{
	...
	// 100
	char *s[2] = {"name", "26 9 23 38 14 52 45 92 87 19 29 63 51 53 25 24 93 77 3 39 28 44 8 12 68 5 37 31 7 95 40 42 100 11 99 96 89 71 79 70 13 80 60 55 57 98 91 62 74 64 56 10 21 54 73 22 34 6 2 49 61 36 48 85 47 84 86 82 81 27 75 33 67 1 41 66 20 32 4 35 88 76 65 17 97 16 50 15 69 46 43 83 90 94 59 30 72 78 58 18"};
	//char *s[2] = {"name", "10 44 80 58 99 48 55 18 89 90 12 83 77 85 64 82 93 59 92 68 43 14 74 95 31 91 13 16 62 78 28 87 70 11 47 22 21 75 49 88 27 9 97 84 8 40 60 54 5 42 69 6 61 67 46 51 57 56 94 39 65 34 19 63 37 66 25 71 50 26 100 17 53 81 36 86 41 20 7 33 32 38 15 35 23 29 2 45 1 3 30 79 4 24 98 73 76 52 72 96"};
